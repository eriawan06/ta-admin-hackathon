...
export default function EditEventManagementComponent() {
  ...
  const Card = ({ title, date, desc, pencilOnclick, closeOnclick }) => {
    return (
      <>
  ...
  useEffect(() => {
    if (id) {
      ...
      dispatch(Event.GetEventDetail(id))
        .unwrap()
        .then((ress) => {
          form.setFieldValue('name', ress?.data?.data?.name)
          form.setFieldValue('start_date', dayjs(ress?.data?.data?.start_date))
          form.setFieldValue('end_date', dayjs(ress?.data?.data?.end_date))
          form.setFieldValue(
            'payment_due_date',
            dayjs(ress?.data?.data?.payment_due_date)
          )
          form.setFieldValue(
            'team_min_member',
            ress?.data?.data?.team_min_member
          )
          form.setFieldValue(
            'team_max_member',
            ress?.data?.data?.team_max_member
          )
          form.setFieldValue('reg_fee', ress?.data?.data?.reg_fee)
          form.setFieldValue('description', ress?.data?.data?.description)
          form.setFieldValue('status', ress?.data?.data?.status)
          notifApi.success({ message: ress.data.message })
          setTimeout(() => {}, 2000)
        })
        .catch((ress) => {
      ...
      dispatch(Event.GetEventAssessment(id))
        .unwrap()
        .then((ress) => {
          notifApi.success({ message: ress.data.message })
          setTimeout(() => {}, 2000)
  ...
  // Function For Edit Event Main Data ==============================
  const onSubmit = (e) => {
    const payload = {
      id: id,
      body: {
    ...
    dispatch(Event.EditEvent(payload))
      .unwrap()
      .then((res) => {
        ...
        <div className='w-full h-fit p-[30px] rounded bg-white drop-shadow-md mb-[22px]'>
          ...
          <div className='grid grid-cols-2  gap-[69px]'>
            ...
            <div className='flex flex-col'>
              ...
              >
                <TextArea style={{ minHeight: '290px' }} />
              </Form.Item>
            </div>
          </div>
          <CustomButton
            ...
            eventMentor?.map((mentor, index) => (
              <React.Fragment key={index}>
                <ImageCard
                  ...
                  closeOnclick={() => DeleteItems(mentor.id, 'Event Mentor')}
                />
              </React.Fragment>
            ))
          ) : (
